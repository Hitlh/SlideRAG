...
def is_empty_parse_error(exc: Exception) -> bool:
    return isinstance(exc, ValueError) and "No content was extracted" in str(exc)


def is_retryable_mineru_error(exc: Exception) -> bool:
    message = str(exc).lower()
    return (
        exc.__class__.__name__ == "MineruExecutionError"
        and (
            "timed out waiting for local mineru-api" in message
            or "bad gateway" in message
            or "failed to query mineru api health" in message
        )
    )
...
async def parse_pages_into_combined_content(
    ...
    for page_info in saved_pages:
        ...
        max_attempts = max(1, get_env_int("PAGE_PARSE_MAX_ATTEMPTS", 3))
        retry_delay_seconds = max(0, get_env_int("PAGE_PARSE_RETRY_DELAY_SECONDS", 10))
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if is_empty_parse_error(exc):
                    print(f"[WARN] Skipping empty parsed page {page_number}: {image_path}")
                    content_list = []
                    parsed_doc_id = None
                    break
                if is_retryable_mineru_error(exc) and attempt < max_attempts:
                    print(
                        f"[WARN] MinerU transient failure on page {page_number}; "
                        f"retrying {attempt + 1}/{max_attempts} after {retry_delay_seconds}s"
                    )
        ...
        page_reports.append(
            {
                ...
                "skipped_empty": len(normalized_items) == 0,
            }
        )

    if not combined_content_list:
        raise ValueError("Parsing failed: all pages produced no content")
