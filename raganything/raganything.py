...
class RAGAnything(QueryMixin, ProcessorMixin, BatchMixin):
    ...
    hidden_expand_cache: Optional[Any] = field(default=None, init=False)
    """Hidden information expansion cache storage using LightRAG KV storage."""
        ...
        try:
            ...
            if self.lightrag is not None:
                ...
                try:
                    ...
                    if self.hidden_expand_cache is None:
                        self.logger.info(
                            "Initializing hidden expansion cache for pre-provided LightRAG instance"
                        )
                        self.hidden_expand_cache = (
                            self.lightrag.key_string_value_json_storage_cls(
                                namespace="hidden_expand_cache",
                                workspace=self.lightrag.workspace,
                                global_config=self.lightrag.__dict__,
                                embedding_func=self.embedding_func,
                            )
                        )
                        await self.hidden_expand_cache.initialize()
            ...
            try:
                ...
                # Initialize hidden expansion cache storage using LightRAG's KV storage
                self.hidden_expand_cache = (
                    self.lightrag.key_string_value_json_storage_cls(
                        namespace="hidden_expand_cache",
                        workspace=self.lightrag.workspace,
                ...
                self.logger.info(
                    "LightRAG, parse/page topic/hidden expansion caches, and multimodal processors initialized"
                )
                return {"success": True}
    ...
    async def finalize_storages(self):
        ...
        try:
            ...
            # Finalize hidden expansion cache if it exists
            if self.hidden_expand_cache is not None:
                tasks.append(self.hidden_expand_cache.finalize())
                self.logger.debug("Scheduled hidden expansion cache finalization")
