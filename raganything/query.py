...
class QueryMixin:
    ...
    async def aquery_vlm_enhanced(
        ...
        # Save messages to JSON for debugging/auditing
        self._save_messages_to_json(messages)
    ...
    def _save_messages_to_json(self, messages: List[Dict]) -> None:
        """Persist VLM messages to JSON file."""
        try:
            base_dir = Path(getattr(self, "working_dir", Path.cwd()))
            output_dir = base_dir / "output"
            output_dir.mkdir(parents=True, exist_ok=True)

            json_file = output_dir / "vlm_messages.json"
            with open(json_file, "w", encoding="utf-8") as file:
                json.dump(messages, file, ensure_ascii=False, indent=2)

            self.logger.info(f"VLM messages saved to: {json_file}")
        except Exception as e:
            self.logger.warning(f"Failed to save VLM messages JSON: {e}")
