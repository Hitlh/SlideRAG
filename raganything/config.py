...
class RAGAnythingConfig:
    ...
    # Hidden Information Expansion Configuration
    # ---
    enable_hidden_expansion: bool = field(
        default=get_env_value("ENABLE_HIDDEN_EXPANSION", True, bool)
    )
    """Enable hidden-information detection and grounded text expansion."""

    hidden_expand_score_threshold: float = field(
        default=get_env_value("HIDDEN_EXPAND_SCORE_THRESHOLD", 0.55, float)
    )
    """Page-level hidden-information score threshold for triggering expansion."""

    hidden_expand_max_items_per_page: int = field(
        default=get_env_value("HIDDEN_EXPAND_MAX_ITEMS_PER_PAGE", 1, int)
    )
    """Maximum number of candidate items expanded on each page."""

    hidden_expand_use_llm: bool = field(
        default=get_env_value("HIDDEN_EXPAND_USE_LLM", True, bool)
    )
    """Whether to use LLM for grounded hidden-information expansion."""

    hidden_expand_max_chars: int = field(
        default=get_env_value("HIDDEN_EXPAND_MAX_CHARS", 1200, int)
    )
    """Maximum page-context characters used in each expansion prompt."""
