...
export class WhatsAppClient {
  ...
  private acceptFromMe = false;

  constructor(options: WhatsAppClientOptions) {
    this.options = options;
    this.proxyAgent = this.buildProxyAgent();
    this.acceptFromMe = String(process.env.WHATSAPP_ACCEPT_FROM_ME || '').toLowerCase() === 'true';
    if (this.acceptFromMe) {
      console.log('Test mode enabled: accepting fromMe messages');
    }
  }

  private buildProxyAgent(): ProxyAgent | null {
    const proxyUrl =
    ...
    try {
      ...
      console.log(`Proxy enabled for Baileys: ${proxyUrl}`);
      return agent;
    } catch (err) {
      console.warn(`Invalid proxy URL "${proxyUrl}", fallback to direct connection`, err);
      return null;
    }
  }
  ...
  async connect(): Promise<void> {
    ...
    const socketConfig: any = {
      ...
      browser: ['sliderag', 'bridge', VERSION],
      syncFullHistory: false,
      markOnlineOnConnect: false,
    };
    if (this.proxyAgent) {
      socketConfig.agent = this.proxyAgent;
      socketConfig.fetchAgent = this.proxyAgent;
    }
    ...
    this.sock.ev.on('messages.upsert', async ({ messages, type }: { messages: any[]; type: string }) => {
      ...
      for (const msg of messages) {
        if (msg.key.fromMe && !this.acceptFromMe) continue;
