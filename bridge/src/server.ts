...
export class BridgeServer {
  ...
  private waStatus: 'connected' | 'disconnected' = 'disconnected';
  ...
  async start(): Promise<void> {
    ...
    this.wa = new WhatsAppClient({
      ...
      onStatus: (status) => {
        this.waStatus = status === 'connected' ? 'connected' : 'disconnected';
        this.broadcast({ type: 'status', status: this.waStatus });
      },
    });
  ...
  private setupClient(ws: WebSocket): void {
    this.clients.add(ws);
    // Replay current WhatsApp connectivity state to late-joining Python clients.
    ws.send(JSON.stringify({ type: 'status', status: this.waStatus }));
