"""Worker process for Feishu runtime.

This module hosts Feishu channel loops and exits with SWITCH_EXIT_CODE when /file switch is requested.
"""
...
from client.feishu_runtime_common import (
    SWITCH_EXIT_CODE,
    RuntimeSettings,
    RuntimeSwitchState,
    build_provider,
...
from rag_agent.channels.feishu import FeishuChannel
...
async def inbound_worker(
    ...
    while True:
        ...
        try:
            ...
            logger.exception("Failed to process inbound Feishu message: {}", exc)
            await bus.publish_outbound(
                OutboundMessage(
                    channel=msg.channel,
                    chat_id=msg.chat_id,
...
async def outbound_worker(bus: MessageBus, channel: FeishuChannel) -> None:
    """Consume outbound queue and deliver through Feishu channel."""
    while True:
...
def _startup_notify_targets(settings: RuntimeSettings) -> list[str]:
    ...
    return [item for item in settings.feishu_config.allow_from if item and item != "*"]


async def send_startup_ready_notification(channel: FeishuChannel, settings: RuntimeSettings) -> None:
    """Send one proactive runtime-ready message after Feishu channel starts."""
    ...
    if not targets:
        logger.warning(
            "Startup notification skipped: configure FEISHU_STARTUP_NOTIFY_CHAT_ID or explicit FEISHU_ALLOW_FROM"
        )
        return
    ...
    try:
        for _ in range(30):
            ...
            logger.warning("Startup notification skipped: Feishu client not ready within timeout")
            return
...
async def maybe_ingest_target_file(rag: object, settings: RuntimeSettings) -> None:
    ...
    if settings.target_file_path is None:
        logger.warning("FEISHU_TARGET_FILE is empty; skipping ingest")
        return

    target = settings.target_file_path.expanduser().resolve()
    if not target.exists():
        raise FileNotFoundError(f"FEISHU_TARGET_FILE does not exist: {target}")
...
async def worker_main(target_file_override: str | None = None) -> int:
    """Start Feishu worker and return process exit code."""
    settings = load_settings(target_file_override=target_file_override)
    if not settings.feishu_config.enabled:
        raise SystemExit("FEISHU_ENABLED is false; nothing to run")
    if not settings.feishu_config.app_id or not settings.feishu_config.app_secret:
    ...
    bus = MessageBus()
    feishu_channel = FeishuChannel(config=settings.feishu_config, bus=bus)

    logger.info("Feishu runtime(worker) starting")
    ...
    out_task = asyncio.create_task(outbound_worker(bus, feishu_channel))
    await send_startup_ready_notification(feishu_channel, settings)
    ...
    try:
        ...
        logger.info("Stopping Feishu runtime(worker)...")
        await feishu_channel.stop()
        for task in (feishu_task, in_task, out_task):
            task.cancel()
        await asyncio.gather(feishu_task, in_task, out_task, return_exceptions=True)
        logger.info("Feishu runtime(worker) stopped")
    return exit_code


def build_worker_cli_args() -> argparse.Namespace:
    """Parse CLI args for direct worker execution."""
    parser = argparse.ArgumentParser(description="Feishu runtime worker process")
