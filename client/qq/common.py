"""Shared configuration and helpers for QQ runtime supervisor/worker.

This module loads runtime settings from `.env` and environment variables.
"""
...
from rag_agent.channels.qq import QQConfig

PROJECT_ROOT = Path(__file__).resolve().parent.parent
QQ_SWITCH_REQUEST_FILE = "qq_runtime_switch_request.json"


@dataclass
class RuntimeSettings(RuntimeCoreSettings):
    """QQ runtime settings loaded from environment."""
...
def load_settings(target_file_override: str | None = None) -> RuntimeSettings:
    ...
    qq_msg_format = get_env_str("QQ_MSG_FORMAT", "plain").lower()
    qq_config = QQConfig(
        enabled=get_env_bool("QQ_ENABLED", True),
        app_id=get_env_str("QQ_APP_ID", ""),
        secret=get_env_str("QQ_SECRET", ""),
        allow_from=parse_allow_from(get_env_str("QQ_ALLOW_FROM", "*")),
        msg_format=qq_msg_format if qq_msg_format in ("plain", "markdown") else "plain",
    )

    target_file_raw = (
        str(target_file_override or "").strip()
        if target_file_override is not None
        else get_env_str("QQ_TARGET_FILE", "")
    )
    ...
    return RuntimeSettings(
        ...
        rag_working_dir=Path(get_env_str("QQ_RAG_WORKING_DIR", "./rag_storage_by_file")).expanduser().resolve(),
        runtime_state_dir=Path(get_env_str("QQ_RUNTIME_STATE_DIR", "./rag_storage_qq_test")).expanduser().resolve(),
        uploaded_docs_dir=Path(get_env_str("QQ_UPLOADED_DOCS_DIR", "./uploaded_docs")).expanduser().resolve(),
        target_file_path=target_file,
        ingest_output_dir=Path(get_env_str("QQ_INGEST_OUTPUT_DIR", "./output")).expanduser().resolve(),
        startup_notify_enabled=get_env_bool("QQ_STARTUP_NOTIFY_ENABLED", True),
        startup_notify_message=get_env_str("QQ_STARTUP_NOTIFY_MESSAGE", "rag agent is ready."),
        startup_notify_chat_id=get_env_str("QQ_STARTUP_NOTIFY_CHAT_ID", ""),
        qq_config=qq_config,
    )
...
def switch_request_path(settings: RuntimeSettings) -> Path:
    ...
    return _switch_request_path(settings.runtime_state_dir, QQ_SWITCH_REQUEST_FILE)
...
def write_switch_request(settings: RuntimeSettings, target_file_path: Path, trigger_chat_id: str) -> None:
    ...
    _write_switch_request(
        ...
        request_file_name=QQ_SWITCH_REQUEST_FILE,
...
def read_switch_request(settings: RuntimeSettings) -> tuple[Path, str] | None:
    ...
    return _read_switch_request(settings.runtime_state_dir, QQ_SWITCH_REQUEST_FILE)
