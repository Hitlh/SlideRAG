"""Shared configuration and helpers for Feishu runtime supervisor/worker."""
...
from rag_agent.channels.feishu import FeishuConfig

PROJECT_ROOT = Path(__file__).resolve().parent.parent
FEISHU_SWITCH_REQUEST_FILE = "feishu_runtime_switch_request.json"


@dataclass
class RuntimeSettings(RuntimeCoreSettings):
    """Feishu runtime settings loaded from environment."""
...
def load_settings(target_file_override: str | None = None) -> RuntimeSettings:
    ...
    feishu_config = FeishuConfig(
        enabled=get_env_bool("FEISHU_ENABLED", True),
        app_id=get_env_str("FEISHU_APP_ID", get_env_str("FEISHU_APPID", "")),
        app_secret=get_env_str("FEISHU_APP_SECRET", get_env_str("FEISHU_APPSECRET", "")),
        encrypt_key=get_env_str("FEISHU_ENCRYPT_KEY", get_env_str("FEISHU_ENCRYPTKEY", "")),
        verification_token=get_env_str(
            "FEISHU_VERIFICATION_TOKEN", get_env_str("FEISHU_VERIFICATIONTOKEN", "")
        ),
        allow_from=parse_allow_from(get_env_str("FEISHU_ALLOW_FROM", "*")),
        domain=get_env_str("FEISHU_DOMAIN", "feishu"),
    )

    target_file_raw = (
        str(target_file_override or "").strip()
        if target_file_override is not None
        else get_env_str("FEISHU_TARGET_FILE", "")
    )
    ...
    return RuntimeSettings(
        ...
        rag_working_dir=Path(get_env_str("FEISHU_RAG_WORKING_DIR", "./rag_storage_by_feishu_file")).expanduser().resolve(),
        runtime_state_dir=Path(get_env_str("FEISHU_RUNTIME_STATE_DIR", "./rag_storage_feishu_runtime")).expanduser().resolve(),
        uploaded_docs_dir=Path(get_env_str("FEISHU_UPLOADED_DOCS_DIR", "./uploaded_docs")).expanduser().resolve(),
        target_file_path=target_file,
        ingest_output_dir=Path(get_env_str("FEISHU_INGEST_OUTPUT_DIR", "./output")).expanduser().resolve(),
        startup_notify_enabled=get_env_bool("FEISHU_STARTUP_NOTIFY_ENABLED", True),
        startup_notify_message=get_env_str("FEISHU_STARTUP_NOTIFY_MESSAGE", "agent is ready."),
        startup_notify_chat_id=get_env_str("FEISHU_STARTUP_NOTIFY_CHAT_ID", ""),
...
def switch_request_path(settings: RuntimeSettings) -> Path:
    ...
    return _switch_request_path(settings.runtime_state_dir, FEISHU_SWITCH_REQUEST_FILE)
...
def write_switch_request(settings: RuntimeSettings, target_file_path: Path, trigger_chat_id: str) -> None:
    ...
    _write_switch_request(
        ...
        request_file_name=FEISHU_SWITCH_REQUEST_FILE,
...
def read_switch_request(settings: RuntimeSettings) -> tuple[Path, str] | None:
    ...
    return _read_switch_request(settings.runtime_state_dir, FEISHU_SWITCH_REQUEST_FILE)
