...
async def _wait_until_feishu_ready(channel: FeishuChannel) -> bool:
    for _ in range(30):
...
def _build_channel(settings, bus: MessageBus) -> FeishuChannel:
    return FeishuChannel(config=settings.feishu_config, bus=bus)


FEISHU_WORKER_HOOKS = WorkerHooks(
    channel_name="feishu",
    channel_display_name="Feishu",
    disabled_flag_env="FEISHU_ENABLED",
    switch_exit_code=SWITCH_EXIT_CODE,
    missing_credentials_message="FEISHU_APP_ID and FEISHU_APP_SECRET are required",
    target_file_env_var="FEISHU_TARGET_FILE",
    ...
    is_enabled=lambda settings: bool(settings.feishu_config.enabled),
    has_required_credentials=lambda settings: bool(
        settings.feishu_config.app_id and settings.feishu_config.app_secret
    ),
    ...
    wait_until_ready=_wait_until_feishu_ready,
)


async def worker_main(target_file_override: str | None = None) -> int:
    """Start Feishu worker and return process exit code."""
    return await worker_main_core(target_file_override=target_file_override, hooks=FEISHU_WORKER_HOOKS)


if __name__ == "__main__":
    args = build_worker_cli_args("Feishu runtime worker process")
