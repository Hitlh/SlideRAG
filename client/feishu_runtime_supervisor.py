...
def run_worker_process(target_file_path: Path | None) -> int:
    """Start worker process and return its exit code."""
    cmd = [sys.executable, "-m", "client.feishu_runtime_worker"]
...
def supervisor_main() -> int:
    ...
    while True:
        ...
        logger.info("Supervisor starting worker with target_file_path={}", current_target)
        exit_code = run_worker_process(current_target)
